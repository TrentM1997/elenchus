...
function Trash({ article, deleteHandler, bookmark }: Trash) {
  return (
    <div className="absolute top-3 right-3 z-10 group">
      ...
      >
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          className={`${bookmark.status === "unbookmarked" ? "text-red-700" : " text-white/80"}
      ...
      {/* Tooltip */}
      <TrashTooltip articleDeleted={bookmark.status === "unbookmarked"} />
    </div>
  );
}
