...
export default function GetInfo({ failedExtraction }) {
    ...
    return (
        <div className={`${failedExtraction ? 'pointer-events-none opacity-30' : 'pointer-events-auto opacity-100 lg:hover:bg-border_gray'}
            flex shrink-0 w-fit h-10 lg:h-auto md:py-1.5 px-2.5 xl:px-2.5 group relative
...
function GetInfoTooltip({ modalActive }: GetInfoTooltipProps): JSX.Element {
    ...
    return (
        <div className={`absolute p-1 bg-white z-50 opacity-0 transition-opacity duration-200 ease-soft 
            ${modalActive ? '' : 'md:group-hover:opacity-100'}
            delay-500  bottom-[3.3rem] -left-5
