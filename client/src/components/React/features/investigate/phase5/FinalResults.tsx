...
export default function FinalResults({
  ...
  idea: PersistInvestigationInputSchemaType["idea"];
}) {
  ...
  return (
    ...
    >
      <div
        className="2xl:max-w-7xl xl:max-w-6xl lg:max-w-5xl md:max-w-4xl sm:max-w-168 h-fit py-16 lg:px-16 md:px-12 xl:px-36 items-center relative w-auto
             max-w-88 bg-gradientdown rounded-[3rem]"
      >
        <div className="text-center max-w-xl mx-auto">
          <span className="text-md 2xl:text-lg text-blue-500 font-light">
            {" "}
            Results
          </span>
          <h2 className="text-sm md:text-xl mt-6 tracking-tight font-light lg:text-2xl text-white">
            Idea Challenged:{" "}
            <span className="block text-zinc-400 mt-2 text-base text-left">
              {" "}
              {idea}
            </span>
          </h2>
          ...
          <div className="relative -mx-8">
            <div className="absolute inset-x-4 inset-y-0 -z-10 flex">
              <div
                ...
                aria-hidden="true"
              >
                <div className="w-full rounded-xl ring-1 ring-white/5 bg-ebony shadow-inset"></div>
              </div>
            </div>
            <DataTable />
          </div>
        ...
        <div className="flex mt-16 gap-x-2 2xl:gap-x-6 w-1/2 justify-center mx-auto">
          <RenderFinalOptionsByUserKind userKind={userKind} />
        </div>
      </div>
    </section>
  );
