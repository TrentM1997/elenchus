...
export default function Stance() {
  ...
  return (
    <section className="w-full h-full xs:px-6 flex flex-col gap-y-1 items-center content-center mx-auto">
      {option === "initial" && (
        <OptionFortakeaway key={"options"} setOption={setOption} />
      )}
      {option === "Opt-in" && (
        <OptionalTakeaway
          key={"input-takeaway"}
...
function OptionalTakeaway({
  newPOV,
  takeAway,
  getTakeAways,
}: OptionalTakeawayProps): JSX.Element | null {
  return (
    <>
