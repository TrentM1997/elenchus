...
export default function DetailView({ backTo }: DetailView): JSX.Element {
  const detailView = (
    <div
      ...
      className="fixed bottom-0 z-40 py-4 w-full px-4 backdrop-blur-xl opacity-0 animate-fade-up will-change-[opacity]
            animation-delay-600ms bg-white/5 md:hover:bg-white/15 transition-colors
             ease-soft backdrop-filter shadow-thick"
    >
      <button
...
function DetailViewArrow(): React.ReactNode {
  return (
    <svg
      ...
      className="icon icon-tabler icons-tabler-outline icon-tabler-chevron-left text-white"
    >
