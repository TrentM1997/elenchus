...
export default function ChartJsSkeleton({
  children,
}: ChartJsSkeleton): JSX.Element | null {
  return (
    <div
      className="relative xl:h-[576px] xl:w-[1045px]
             lg:p-8 opacity-0 animate-fade-in transition-opacity animation-delay-300ms"
    >
      <ChartSkeletonWrapper>{children}</ChartSkeletonWrapper>
    </div>
  );
}
...
export function ChartSkeletonWrapper({
  children,
}: ChartSkeletonWrapper): JSX.Element | null {
  return (
...
export function SkeletonDonut(): JSX.Element | null {
  return (
    <div className="relative md:h-88 md:w-88 h-72 w-72 rounded-full border-[88px] border-zinc-800/50  border-t-zinc-800/30 border-l-zinc-800 transform-cpu will-change-transform animate-pulse" />
  );
}
  ...
  return (
    ...
    >
      <div className="space-y-4 md:space-y-2">
        ...
        <div
          className="h-4 md:h-5 w-40 md:w-48 rounded-md bg-[length:200%_100%] transform-cpu will-change-transform
    bg-[linear-gradient(110deg,#1a1c23_8%,#2b2f3a_18%,#1a1c23_33%)] animate-shimmer"
        />

        {/* Title: "Source diversity" */}
        <div
          className="h-10 md:h-14 w-2/3 rounded-md bg-[length:200%_100%] transform-cpu will-change-transform
    bg-[linear-gradient(110deg,#1a1c23_8%,#2b2f3a_18%,#1a1c23_33%)] animate-shimmer"
        />
        ...
        <div className="space-y-2">
          <div
            className="h-4 md:h-5 w-full md:w-4/5 rounded-md bg-[length:200%_100%] transform-cpu will-change-transform
    bg-[linear-gradient(110deg,#1a1c23_8%,#2b2f3a_18%,#1a1c23_33%)] animate-shimmer"
          />
          <div
            className="h-4 md:h-5 w-2/3 md:w-1/2 rounded-md bg-[length:200%_100%] transform-cpu will-change-transform
    bg-[linear-gradient(110deg,#1a1c23_8%,#2b2f3a_18%,#1a1c23_33%)] animate-shimmer"
          />
        </div>
          ...
          {["w-72 md:w-96", "w-80 md:w-[28rem]", "w-72 md:w-[26rem]"].map(
            (w, i) => (
              <li key={i} className="flex items-center gap-3 md:gap-4">
                ...
                <span
                  className="inline-block h-4 w-4 md:h-5 md:w-5 rounded-full bg-[length:200%_100%] transform-cpu will-change-transform
    bg-[linear-gradient(110deg,#1a1c23_8%,#2b2f3a_18%,#1a1c23_33%)] animate-shimmer"
                />
                <span
                  className={`block h-4 md:h-5 rounded-md bg-[length:200%_100%] transform-cpu will-change-transform
