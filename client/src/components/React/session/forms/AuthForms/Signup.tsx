...
export default function Signup() {
  ...
  const { fieldStatus, setFieldValue, canSubmit, fields } =
    useSignupValidation();
  ...
  const submitNewUserRequest = async (
  ...
  return (
    <section className="lg:p-8 min-h-dvh overflow-hidden bg-black animate-fade-in relative">
      <div className="mx-auto 2xl:max-w-7xl py-12 sm:py-24 lg:px-16 md:px-12 px-8 xl:px-36">
        ...
        {/*   ERROR MESSAGE HERE  */}
        <motion.div className="w-full gap-6 sm:gap-24 mx-auto grid grid-cols-1 mt-12 lg:grid-cols-2 items-start relative">
          <form>
            <div className="space-y-4">
              <NewEmail
                emailStatus={fieldStatus.e}
                setFieldValue={setFieldValue}
              />
              <NewPassword
                setFieldValue={setFieldValue}
                passwordStatus={fieldStatus.p}
              />
              <ConfirmSignupPassword
                confirmStatus={fieldStatus.c}
                setFieldValue={setFieldValue}
              />
              ...
              <div>
                <p className="font-medium text-sm leading-tight text-white mx-auto">
                  ...
                  Already a member?
                  <Link
                    className="text-white underline hover:text-blue-400 ml-3"
                    to={"/Login"}
                  >
                    Log in now
                  </Link>
