...
import type { NewPasswordStatus } from "@/hooks/auth/useValidateNewPassword"
import type { SignupValidationHook } from "@/env"
...
export function ConfirmSignupPassword({ confirmStatus, setFieldValue }: Confirm) {
    ...
    return (
            ...
            <div>
                <label htmlFor="confirm_password" className="block mb-2 md:mb-3 text-sm font-medium text-white">
                    <div className="flex justify-between pr-2 gap-x-6 items-center">
                        ...
                        <div className="w-fit">
                            {(confirmStatus === 'valid') && <svg xmlns="http://www.w3.org/2000/svg" className={`icon icon-tabler icon-tabler-check text-green-500 mx-auto`} width={16} height={16} viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                <path d="M5 12l5 5l10 -10" />
                            </svg>}
                            {(confirmStatus === 'rejected') && <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline text-red-500 icon-tabler-x"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M18 6l-12 12" /><path d="M6 6l12 12" /></svg>
        ...
        ${(confirmStatus === 'rejected') && 'border-red-500 focus:border-red-500'}
        ${(confirmStatus === 'valid') && 'border-green-500 focus:border-green-500'}
         ${(confirmStatus === 'idle') && 'focus:border-white/5 '}
        `} required />
