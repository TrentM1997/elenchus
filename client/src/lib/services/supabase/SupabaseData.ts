...
import {
  ...
  createSigninRequest,
  createUserRequest,
} from "@/infra/transport/request/apiClient";
...
export const newUser = async (
  ...
  try {
    ...
    if (!ok) {
      logValidationError(errors);
      throw new Error(
        "Failed to validate email and password schema for new user creation",
      );
    }
...
export const getInvestigationSources = (
  sources: string[],
  savedArticles: Array<Article>,
) => {
  if (!sources || !Array.isArray(sources) || savedArticles.length === 0) return;

  const sourceSet = new Set(sources);

  return savedArticles.filter((article: Article) =>
    sourceSet.has(article.article_url),
  );
};
...
export const submitFeedback = async (
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Couldn't reach the endpoint for feedback: ${response.statusText}`,
      );
    }
