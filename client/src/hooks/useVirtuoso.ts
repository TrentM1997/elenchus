...
interface VirtuosoHook<T> {
  ...
  loadMore: LoadMore;
  numSkeletons: number;
  scrollRef: MutableRefObject<HTMLElement | Window | null>;
  topKeyRef: MutableRefObject<number | null>;
  topIndexRef: MutableRefObject<number | null>;
  saveNow: () => void;
...
type RestoreScrollToken = Extract<
  VirtuosoScrollPos,
  { status: "ready" }
>["position"];

export function useVirtuoso<T>(
  items: T[],
  scrollerId?: ScrollID,
  ...
  const restoreInfo = useMemo(() => {
    if (
      !restoreToken ||
      restoreToken.listID !== scrollerId ||
      didSaveRef.current
    )
      return null;
    ...
    restoreToken?.savedAt,
    restoreToken?.listID,
    restoreToken?.topKey,
    restoreToken?.topIndex,
    restoreToken?.dataVersion,
    restoreToken?.scrollTop,
    scrollerId,
    items,
  ]);
