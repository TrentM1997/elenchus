...
export function cleanMarkdownArticle(markdown) {
    ...
    for (const block of rawBlocks) {
        ...
        if (looksLikeNewsletterCTA(trimmed))
            continue;
        if (looksLikeShareOrFollow(trimmed))
            continue;
        if (looksLikeRelatedLinksBlock(trimmed))
            continue;
        if (looksLikeCommentsBlock(trimmed))
            continue;
        if (looksLikeNavOrUtilityBlock(trimmed))
            continue;
...
function looksLikeNewsletterCTA(block) {
...
function looksLikeShareOrFollow(block) {
    const lower = block.toLowerCase();
    // We treat any "follow us on X/FB/etc" as junk.
    if (lower.includes("follow us on") ||
        lower.includes("follow ") && (lower.includes("twitter") ||
    ...
    // share prompts
    if (lower.includes("share this article") ||
...
function looksLikeRelatedLinksBlock(block) {
    const lower = block.toLowerCase();
    ...
    if (startsLikeLinkHub.test(block))
        return true;
    ...
    const linkyLines = lines.filter(l => /^\s*[-*+]\s*\[.+?\]\(.+?\)/.test(l.trim()) || // bullet markdown links
...
function looksLikeCommentsBlock(block) {
    const lower = block.toLowerCase();
    if (lower.includes("leave a comment") ||
...
function looksLikeNavOrUtilityBlock(block) {
    const lower = block.toLowerCase();
    // breadcrumb-y / section nav style:
    // "Home / World / Business / Markets"
    if (/home\s*\/\s*\w+/.test(lower) &&
        /\/\s*\w+/.test(lower)) {
        return true;
    }
...
function looksLikeEndOfArticle(block) {
    const lower = block.toLowerCase();
